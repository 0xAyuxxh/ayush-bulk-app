...
with open("C:/Users/ASUS/.gemini/antigravity/scratch/ayush-bulk-app/src/BulkApp.jsx", "r", encoding="utf-8") as f:
    content = f.read()
...
# 2. Macros
macros_orig = """  const eatenP = dayPlan.reduce((s, m, i) => s + (dayChecked[i] ? m.protein : 0), 0);
  const eatenC = dayPlan.reduce((s, m, i) => s + (dayChecked[i] ? m.cal : 0), 0);"""
macros_new = macros_orig + """
...
# 3. HomePage
homepage_orig = content[content.find("  // ── HOME PAGE"):content.find("  // ── MODALS ──")]
homepage_new = """  // ── HOME PAGE ────────────────────────────────────────────────────────────
  const HomePage = () => {
...
# 4. Nav
nav_orig = content[content.find("      {/* Floating Bottom Nav */}"):content.find("    </div>\n  );\n}\n") + len("    </div>\n  );\n}\n")]
nav_new = """      {/* Floating Bottom Nav matched to the image */}
...
with open("C:/Users/ASUS/.gemini/antigravity/scratch/ayush-bulk-app/src/BulkApp.jsx", "w", encoding="utf-8") as f:
    f.write(content)
